  ...
  const uniqueFeatured = Array.from(
    ...
    useEffect(()=>{
      const fetchPub = async ()=>{
        const slug = "the-nilar-the-path-to-african-economic-sovereignty-and-prosperity"
...
console.log(publicationData)
  ...
  Publications & Research
    ...
    <section className={`bg-white text-gray-800 mt-10 px-6 sm:px-10  ${ibmPlexSans.className}`}>
      <div className="max-w-6xl mx-auto">
        ...
        {publicationData.map(
            (
              ...
              const slugData = uniqueFeatured[0]
    ...
    backgroundImage: `url('${image}')`,
                    ...
                    <div className="flex flex-col  w-full  md:w-8/12 gap-y-4 md:gap-y-6">
                      ...
                      <div className="flex flex-col gap-y-4">
                        ...
                        <a  className="flex items-center justify-center gap-3 border-2 bg-deepForest border-[#00210d] dark:border-yellow-400 text-[#ffd700] dark:text-yellow-300  dark:hover:bg-yellow-400 hover:text-deepForest hover:bg-white dark:hover:text-black font-semibold py-3 px-6 rounded-xl shadow-md transition duration-300" href={`${paths.publications}/${slugData?.slug}?type=${category}`}>Read Paper</a>
                      </div>
                    </div>
                  </div>
